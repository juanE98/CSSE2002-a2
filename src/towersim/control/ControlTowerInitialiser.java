...
public class ControlTowerInitialiser {
    ...
    /**
     * Loads the number of ticks elapsed from the given reader instance. The contents of the reader
     * should match the format specified in the tickWriter row of in the  table shown in
     * ViewModel.saveAs().
     * <p>
     ...
     * (String)). The number of ticks elapsed is less than zero.
     *
     ...
     * cannot be parsed by Integer.parseInt(String)). The number of aircraft specified on the first
     * line is not equal to the number of aircraft actually read from the reader. Any of the
     * conditions listed in the Javadoc for readAircraft(String) are true. This method should call
     * readAircraft(String).
     *
    ...
    /**
     * Reads an aircraft from its encoded representation in the given string. If the
     * AircraftCharacteristics.passengerCapacity of the encoded aircraft is greater than zero,  then
     * a PassengerAircraft should be created and returned. Otherwise, a FreightAircraft  should be
     ...
     * <p>
     * More/fewer colons (:) are detected in the string than expected. The aircraft's
     * AircraftCharacteristics is not valid, i.e. it is not one of those listed in
     * AircraftCharacteristics.values(). The aircraft's fuel amount is not a double (i.e. cannot be
     * parsed by  Double.parseDouble (String)). The aircraft's fuel amount is less than zero or
     * greater than the aircraft's maximum fuel capacity. The amount of cargo (freight/passengers)
     * onboard the aircraft is not an integer (i.e. cannot  be parsed by Integer.parseInt(String)).
     * The amount of cargo (freight/passengers) onboard the aircraft is less than zero or greater
     * than the aircraft's maximum freight/passenger capacity. Any of the conditions listed in the
     * Javadoc for readTaskList(String) are true.
     *
    ...
    /**
     * Reads a task list from its encoded representation in the given string. The format of the
     * string should match the encoded representation of a task list, as described  in
     * TaskList.encode().
     * <p>
     * The encoded string is invalid if any of the following conditions are true:
     * <p>
     * The task list's TaskType is not valid (i.e. it is not one of those listed in TaskType
     * .values()). A task's load percentage is not an integer (i.e. cannot be parsed by
     * Integer.parseInt (String)). A task's load percentage is less than zero. More than one
     * at-symbol (@) is detected for any task in the task list. The task list is invalid according
     * to the rules specified in TaskList(List).
     *
     ...
     * instance. Rather than returning a list of queues, this method does not return anything.
     * Instead,  it should modify the given takeoff queue, landing queue and loading map by adding
     * aircraft, etc.
     * <p>
    ...
    /**
     * Reads an aircraft queue from the given reader instance. Rather than returning a queue, this
     * method does not return anything. Instead, it  should modify the given aircraft queue by
     * adding aircraft to it.
     * <p>
     ...
     * The contents read from the reader are invalid if any of the following conditions are true:
     * <p>
     * The first line read from the reader is null. The first line contains more/fewer colons (:)
     * than expected. The queue type specified in the first line is not equal to the simple class
     * name of the queue provided as a parameter. The number of aircraft specified on the first line
     * is not an integer (i.e. cannot be parsed  by Integer.parseInt(String)). The number of
     * aircraft specified is greater than zero and the second line read is null. The number of
     * callsigns listed on the second line is not equal to the number of aircraft specified on the
     * first line. A callsign listed on the second line does not correspond to the callsign of any
    ...
    public static void readQueue(BufferedReader reader, List<Aircraft> aircraft,
        ...
        //number of aircraft listed on file
        int numAircraft;
        //actual number of aircrafts read from file
    ...
    /**
     * Reads the map of currently loading aircraft from the given reader instance. Rather than
     * returning a map, this method does not return anything. Instead, it should modify  the given
     * map by adding entries (aircraft/integer pairs) to it.
     * <p>
     ...
     * The contents read from the reader are invalid if any of the following conditions are true:
     * <p>
     * The first line read from the reader is null. The number of colons (:) detected on the first
     * line is more/fewer than expected. The number of aircraft specified on the first line is not
     * an integer (i.e. cannot be parsed  by Integer.parseInt(String)). The number of aircraft is
     * greater than zero and the second line read from the reader is null. The number of aircraft
     * specified on the first line is not equal to the number of callsigns read on the second line.
     * For any callsign/loading time pair on the second line, the number of colons detected is not
     * equal to one. For example, ABC123:5:9 is invalid. A callsign listed on the second line does
     * not correspond to the callsign of any aircraft contained in the list of aircraft given as a
     * parameter. Any ticksRemaining value on the second line is not an integer (i.e. cannot be
     * parsed by Integer.parseInt(String)). Any ticksRemaining value on the second line is less than
     * one (1).
     *
    ...
    /**
     * Loads the list of terminals and their gates from the given reader instance. The contents of
     * the reader should match the format specified in the terminalsWithGatesWriter  row of in the
     ...
     * The contents read from the reader are invalid if any of the following conditions are true:
     * <p>
     * The number of terminals specified at the top of the file is not an integer  (i.e. cannot be
     * parsed by Integer.parseInt(String)). The number of terminals specified is not equal to the
     * number of terminals actually read from the reader. Any of the conditions listed in the
     * Javadoc for readTerminal(String, BufferedReader, List) and readGate(String, List) are true.
    ...
    public static List<Terminal> loadTerminalsWithGates(Reader reader, List<Aircraft> aircraft)
        ...
        //number of terminals specified on file
        int numTerminals = 0;
        BufferedReader file = new BufferedReader(reader);
        try {
     ...
     * <p>
     * The number of colons (:) detected on the first line is more/fewer than expected. The terminal
     * type specified on the first line is neither AirplaneTerminal nor HelicopterTerminal. The
     * terminal number is not an integer (i.e. cannot be parsed by Integer.parseInt(String)). The
     * terminal number is less than one (1). The number of gates in the terminal is not an integer.
     * The number of gates is less than zero or is greater than Terminal.MAX_NUM_GATES. A line
     * containing an encoded gate was expected, but EOF (end of file) was received (i.e.
     * BufferedReader.readLine() returns null). Any of the conditions listed in the Javadoc for
     * readGate(String, List) are true.
     *
    ...
    /**
     * Reads a gate from its encoded representation in the given string. The format of the string
     * should match the encoded representation of a gate, as described in  Gate.encode().
     * <p>
     * The encoded string is invalid if any of the following conditions are true:
     * <p>
     * The number of colons (:) detected was more/fewer than expected. The gate number is not an
     * integer (i.e. cannot be parsed by Integer.parseInt(String)). The gate number is less than one
     * (1). The callsign of the aircraft parked at the gate is not empty and the callsign does not
     ...
     * <p>
     * loadTick(Reader) to load the number of elapsed ticks loadAircraft(Reader) to load the list of
     * all aircraft loadTerminalsWithGates(Reader, List) to load the terminals and their gates
     * loadQueues(Reader, List, TakeoffQueue, LandingQueue, Map) to load the takeoff queue, landing
     * queue and map of loading aircraft to their loading time remaining Note: before calling
     * loadQueues(), an empty takeoff queue and landing queue should be created  by calling their
     * respective constructors. Additionally, an empty map should be created by calling:
     * <p>
     * new TreeMap<>(Comparator.comparing(Aircraft::getCallsign)) This is important as it will
     * ensure that the map is ordered by aircraft callsign (lexicographically). Once all information
     * has been read from the readers, a new control tower should be initialised  by calling
     * ControlTower(long, List, LandingQueue, TakeoffQueue, Map). Finally,  the terminals that have
     * been read should be added to the control tower by calling  ControlTower.addTerminal
     * (Terminal).
     *
