...
/**
 * Represents a rule-based queue of aircraft waiting in the air to land. The rules in the landing
 * queue are designed to ensure that aircraft are prioritised for landing based on "urgency" factors
 * such as remaining fuel onboard, emergency status and cargo type.
 */
public class LandingQueue extends AircraftQueue {
     ...
     * the queue is empty. The rules for determining which aircraft in the queue should be returned
     * next are as follows:
     * <p>
     * If an aircraft is currently in a state of emergency, it should be returned. If more than one
     * aircraft are in an emergency, return the one added to the queue first. If an aircraft has
     * less than or equal to 20 percent fuel remaining, a critical level, it should be returned (see
     * Aircraft.getFuelPercentRemaining()). If more than one aircraft have  a critical level of fuel
     * onboard, return the one added to the queue first. If there are any passenger aircraft in the
     * queue, return the passenger aircraft that was added to the queue first. If this point is
     * reached and no aircraft has been returned, return the aircraft that was added to the queue
     * first.
     *
     ...
     * empty. The same rules as described in peekAircraft() should be used for determining which
