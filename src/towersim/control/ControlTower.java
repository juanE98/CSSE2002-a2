 ...
 * and departures in/out of the airport, as well as aircraft that need to be loaded with cargo at
 * gates in terminals.
 *
 * @ass1
 */
public class ControlTower implements Tickable {
    ...
    /**
     * Creates a new ControlTower. The number of ticks elapsed, list of aircraft, landing queue,
     * takeoff queue and map of loading aircraft to loading times should all be set to the values
     ...
     * suitable gate as found by the {@link #findUnoccupiedGate(Aircraft)} method. If there is no
     * suitable gate for the aircraft, the {@code NoSuitableGateException} thrown by {@code
     * findUnoccupiedGate()} should be propagated out of this method.
     * <p>
     * After the aircraft has been added, it should be placed in the appropriate queues by calling
     * placeAircraftInQueues.
     *
     ...
     * <p>
     * The order in which aircraft appear in this list should be the same as the order in which they
     * were added by calling {@link #addAircraft(Aircraft)}.
     * <p>
     * Adding or removing elements from the returned list should not affect the original list.
     *
     * @return all aircraft
     ...
     * <p>
     * Only terminals of the same type as the aircraft's AircraftType (see {@link
     * towersim.aircraft.AircraftCharacteristics#type}) should be considered. For example, for an
     * aircraft with an AircraftType of {@code AIRPLANE}, only AirplaneTerminals may be considered.
     * <p>
     * For each compatible terminal, the {@link Terminal#findUnoccupiedGate()} method should be
     * called to attempt to find an unoccupied gate in that terminal. If {@code
     * findUnoccupiedGate()} does not find a suitable gate, the next compatible terminal in the
     * order they were added should be checked instead, and so on.
     * <p>
     * If no unoccupied gates could be found across all compatible terminals, a {@code
     * NoSuitableGateException} should be thrown.
     *
    ...
    /**
     * Attempts to land one aircraft waiting in the landing queue and park it at a suitable gate. If
     * there are no aircraft in the landing queue waiting to land, then the method should return
    ...
    /**
     * Attempts to allow one aircraft waiting in the takeoff queue to take off. If there are no
     * aircraft waiting in the takeoff queue, then the method should return. Otherwise, the aircraft
     * at the front of the takeoff queue should be removed from the queue and it should move to the
     * next task in its task list.
     */
     ...
     * from their gate once finished loading. Any aircraft in the loading map should have their time
     * remaining decremented by one tick. If any aircraft's time remaining is now zero, it has
     * finished loading and should be removed  from the loading map. Additionally, it should leave
     * the gate it is parked at and should move on to its next task.
     */
    ...
    /**
     * Moves the given aircraft to the appropriate queue based on its current task. If the
     * aircraft's current task type is LAND and the landing queue does not already contain the
     * aircraft, it should be added to the landing queue. If the aircraft's current task type is
     * TAKEOFF and the takeoff queue does not already contain  the aircraft, it should be added to
     * the takeoff queue. If the aircraft's current task type is LOAD and the loading map does not
     * already contain the  aircraft, it should be added to the loading map with an associated value
     * of  Aircraft .getLoadingTime() (this is the number of ticks it will remain in the loading
     * phase).
     *
    ...
    /**
     * Finds the gate where the given aircraft is parked, and returns null if the aircraft is not
     ...
     * <p>
     * Move all aircraft with a current task type of AWAY or WAIT to their next task. Process
     * loading aircraft by calling loadAircraft(). On every second tick, attempt to land an aircraft
     * by calling tryLandAircraft().  If an aircraft cannot be landed, attempt to allow an aircraft
     * to take off instead by calling tryTakeOffAircraft(). Note that this begins from the second
     * time tick() is called and every  second tick thereafter. If this is not a tick where the
     * control tower is attempting to land an aircraft, an aircraft  should be allowed to take off
     * instead. This ensures that aircraft wishing to take  off and land are given an equal share of
     * the runway. Place all aircraft in their appropriate queues by calling
     * placeAllAircraftInQueues(). Note that the actions performed by {@code tick()} are very simple
    ...
    public void tick() {
        ...
        this.loadAircraft();
        //attempt to land aircraft on every second tick.
        if (ticksElapsed % 2 == 0) {
    ...
    /**
     * Returns the human-readable string representation of this control tower. The format of the
     * string to return is
     * <p>
     * ControlTower: numTerminals terminals, numAircraft total aircraft (numLanding LAND, numTakeoff
     *  TAKEOFF, numLoad LOAD) where numTerminals is the number of terminals, numAircraft is the
     * number of aircraft, numLanding is the number of aircraft in the landing queue, numTakeoff is
     * the number of aircraft in the takeoff queue, and numLoad is the number of aircraft in the
     * loading map.
     * <p>
